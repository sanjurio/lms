...
class User(UserMixin, db.Model):
    ...
    def set_access_based_on_domain(self):
        """Set user access level based on email domain if not already set"""
        from .utils.auth_helpers import get_domain_access_info
        if self.email:
            domain = self.email.split('@')[-1].lower()
            self.email_domain = domain
            
            # Only set access_level if it's currently at default (D1/1)
            # or if it's one of the old string types
            if self.access_level in [1, 'basic', 'text_only', 'full_access']:
                access_info = get_domain_access_info(self.email)
                # Map old string levels to integers if necessary
                new_level = access_info.get('access_level', 'basic')
                if isinstance(new_level, str):
                    level_map = {'basic': 1, 'text_only': 2, 'full_access': 3}
                    self.access_level = level_map.get(new_level, 1)
                else:
...
class UserActivity(db.Model):
    ...
    # Relationships
    user = db.relationship('User', backref=db.backref('user_activities', lazy='dynamic', viewonly=True), foreign_keys=[user_id])
